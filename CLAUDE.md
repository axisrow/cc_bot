...
`app/handlers.py`: `/start`, `/id`, `/status`, плюс **catch-all `any_message`** — на любое
сообщение в личном чате отвечает результатом парсинга (через общий `_reply_status`, как `/status`).
Хендлеры матчатся в порядке регистрации, поэтому **catch-all обязан оставаться последним**, иначе
перехватит команды. Фильтр `F.chat.type == "private"` не даёт боту отвечать в групповом чате
уведомлений.
